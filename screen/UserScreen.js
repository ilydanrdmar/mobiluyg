...
{modalVisible && selectedTestDetails && (
  ...
  >
    ...
    <Text>
      Geo Min - Max: 
      {evaluation.range?.geoMin && evaluation.range?.geoMax 
        ? `${evaluation.status} (Min: ${evaluation.range?.geoMin} - Max: ${evaluation.range?.geoMax})`
        : "Veri Yok"}
    </Text>
    ...
    <Text>
      Mean Min - Max: 
      {evaluation.range?.meanMin && evaluation.range?.meanMax 
        ? `${evaluation.status} (Min: ${evaluation.range?.meanMin} - Max: ${evaluation.range?.meanMax})`
        : "Veri Yok"}
    </Text>

    {/* Interval Min - Max Durumu */}
    <Text>
      Interval Min - Max: 
      {evaluation.range?.interval?.[0] && evaluation.range?.interval?.[1]
        ? `${evaluation.status} (Min: ${evaluation.range?.interval[0]} - Max: ${evaluation.range?.interval[1]})`
        : "Veri Yok"}
    </Text>
